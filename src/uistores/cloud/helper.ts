...
export const conversionOption = (ext: string, scale?: number): ConversionOption | undefined => {
  const needConvert = convertableTypes.includes(toLower(ext));
  const needDynamicConvert = convertableDynamicTypes.includes(toLower(ext));
...
export const fileExt2ContentType = (fileExt: string) => {
  return (
    {
      '.323': 'text/h323',
      '.3gp': 'video/3gpp',
